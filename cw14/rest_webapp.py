...
class OsobyApp:
    ...
    def route(self):
        ...
        if m is not None:
            ...
            return
        p = self.env['PATH_INFO']
        if(p == '/osoby/search'):
            query = self.env['QUERY_STRING']
            vals = query.split('&')
            vals[0] = vals[0][vals[0].find('=')+1:]
            vals[1] = vals[1][vals[1].find('=')+1:]
            self.handle_search(urllib.parse.unquote(vals[0]), urllib.parse.unquote(vals[1]))
            return
    ...
    def handle_search(self, imie, nazwisko):
        if self.env['REQUEST_METHOD'] == 'GET':
            colnames, rows = self.sql_search(imie, nazwisko)
            if len(rows) == 0:
                self.failure('404 Not Found')
            else:
                self.send_rows(colnames, rows)
    ...
    def sql_search(self, imie, nazwisko):
        conn = sqlite3.connect(plik_bazy)
        crsr = conn.cursor()
        query = 'SELECT * FROM osoby WHERE (imie = \'' + str(imie) + '\' AND nazwisko = \'' + str(nazwisko) + '\')'
        crsr.execute(query)
        colnames = [ d[0] for d in crsr.description ]
        rows = crsr.fetchall()
        crsr.close()
